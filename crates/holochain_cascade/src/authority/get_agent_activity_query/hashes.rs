                ...
                if state.status.is_none() {
                    ...
                    if let Some(fork) = fork {
                        state.status = Some(ChainStatus::Forked(ChainFork {
                            fork_seq: seq,
                            first_action: action.item_hash().clone(),
                            second_action: fork.item_hash().clone(),
                        }));
                    }
                }
...
fn compute_highest_observed(state: &State) -> Option<HighestObserved> {
    ...
    if let Some(valid) = state.valid.last() {
        check_highest(valid.action_seq(), valid.item_hash());
    }
    if let Some(rejected) = state.rejected.last() {
        check_highest(rejected.action_seq(), rejected.item_hash());
    }
    if let Some(pending) = state.pending.last() {
        check_highest(pending.action_seq(), pending.item_hash());
    }
